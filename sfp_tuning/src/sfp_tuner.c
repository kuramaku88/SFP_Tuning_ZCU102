/***************************** 
Author: S.Mullai Vaneshwar
Description: Uses I2C to communicate with SFP modules connected to the SFP cages on the ZCU102.
Date: 31/05/2023
*********************************/
 ...
 */
#define MAX_SIZE	2
#define PAGE_SIZE_16	2
#define PAGE_SIZE_32	2
...
u16 PageSlct = 1;
u32 PageSize;
/* The desired wavelength should be entered as a hex value in the following bytes*/
u16 wl_msb = 0x78;
u16 wl_lsb = 0x6c;
...
/*****************************************************************************/
/**
* This function writes, reads the password 4 bytes at one time.
*
* @param	None.
*
* @return	XST_SUCCESS if successful else XST_FAILURE.
...
//  Changing address to write new wavelength
	WriteBuffer[0] = (u8) (Address + 146);
	...
	if (Status != XST_SUCCESS) {
		...
		xil_printf("Read value before writing at 146: %x \r\n", ReadBuffer[0]);
		xil_printf("Read value before writing at 147: %x \r\n", ReadBuffer[1]);
	}
//  The 4 byte Hexadecimal password is stored as shown below:
	WriteBuffer[1] = wl_msb;
	WriteBuffer[2] = wl_lsb;

//  Writes the 2 byte wavelength
...
//  Reading bytes 123-126 after writing the password
	Status = EepromReadData(&IicInstance, ReadBuffer, PageSize);
	if (Status != XST_SUCCESS) {
		xil_printf("Failed at read\r\n");
		return XST_FAILURE;
		...
		xil_printf("Read value after writing at 146: %x \r\n", ReadBuffer[0]);
		xil_printf("Read value after writing at 147: %x \r\n", ReadBuffer[1]);
	}
